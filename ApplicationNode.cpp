...
ExpNode* ApplicationNode::eval() {
    ...
    if (dynamic_cast<AbstractionNode*>(leftEval)) { //REDEX
        ...
        ExpNode* substituted = absNode->getBody()->substitute(absNode->getVar()->getName(), rightEval); // first evaluates the right expression, then substitutes
...
std::string ApplicationNode::toString() const {
    return "(" + lexp->toString() + " " + rexp->toString() + ")";
}